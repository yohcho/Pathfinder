...
function App() {
  ...
  const beginPathFind=async()=>{
    ...
    while(searchList.length>0){
      ...
      if(addNeighbors(searchList,paths,current)) break
    }

    let searchIndex = 0;
    const searchInterval = setInterval(()=>{
                      const tile = allSearch[searchIndex]
                      if(searchIndex===allSearch.length-1 || (tile[0]===endPos[0] && tile[1]===endPos[1])){
                        clearInterval(searchInterval)
                        finishUp(current,paths)
                      }
                      changeColor(tile,"searchedPath")
                      searchIndex++
                    },1)
  }
    ...
    for(let i = 0;i<maze.length;i++){
      ...
      for(let j = 0;j<maze[0].length;j++){
        ...
        if(maze[i][j]==="searchedPath")
          bg="green"
        row.push(
          <div style={{backgroundColor:bg}} className='maze-row-tile' onClick={()=>handleTileClicked(i,j,selection)}>
            {content}
          </div>
        )
      }
